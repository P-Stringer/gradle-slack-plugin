...
class SlackPlugin implements Plugin<Project> {
    ...
    void handleTaskFinished(Task task, TaskState state) {
        ...
        boolean shouldSendMessage = true; //failure != null || shouldMonitorTask(task);

        // only send a slack message if the task failed
        // or the task is registered to be monitored
        if (shouldSendMessage) {
            SlackMessage slackMessage = SlackMessageTransformer.buildSlackMessage(mExtension.title, task, state, mExtension.channel, mTaskLogBuilder.toString())
